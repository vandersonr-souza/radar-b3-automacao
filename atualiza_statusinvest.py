...
print("1/3 Baixando TODAS as Ações da B3 do Status Invest...")
...
try:
    ...
    acoes_data = data_raw.get("list", []) if isinstance(data_raw, dict) else (data_raw if isinstance(data_raw, list) else [])
except Exception as e:
    print(f"Erro ao baixar ações: {e}")
    acoes_data = []

print(f"-> {len(acoes_data)} ações obtidas.")

print("2/3 Baixando TODOS os FIIs da B3 do Status Invest...")
...
try:
    ...
    fiis_data = data_raw.get("list", []) if isinstance(data_raw, dict) else (data_raw if isinstance(data_raw, list) else [])
except Exception as e:
    print(f"Erro ao baixar FIIs: {e}")
    fiis_data = []
...
for item in acoes_data:
    ...
    if not coerente:
        ...
        if is_best:
            ...
            motivo = f"Setor BEST perene com DY de {dy:.1f}% e desconto Bazin até R$ {bazin_max:.2f}."
        else:
...
for item in fiis_data:
    ...
    if is_papel:
        if p_vp <= 1.00 and dy >= 10.0:
            ...
            motivo = f"Risco de Ágio: P/VP de {p_vp:.2f} em dívida prefixada/IPCA."
        else:
            status_compra = "NEUTRO"
        ...
        if p_vp < 0.95:
            status_compra = "BOM PARA COMPRA"
            motivo = f"Desconto patrimonial em Tijolo ({((1-p_vp)*100):.1f}% de desconto sobre laudo)."
        elif p_vp > 1.05:
            status_compra = "CARO / AGUARDAR"
            motivo = f"Ágio sobre valor dos imóveis (P/VP {p_vp:.2f})."
        else:
            status_compra = "PREÇO JUSTO"
            motivo = f"P/VP equilibrado ({p_vp:.2f})."
...
print(f"3/3 Enviando {len(payload)} ativos consolidados para o Supabase...")
batch_size = 50
...
print("\nCarga completa finalizada com sucesso! Todos os ativos da B3 estão no Supabase.")
